...
# was never written.
set -euo pipefail
# call with a full filename of a procedure in the ./procedures directory
...
if [ ! -d "$PROCEDURES_DIR" ]; then
    echo "Error: Directory '$PROCEDURES_DIR' does not exist."
    exit 1
fi

# Build the full path to the procedure file
PROCEDURE_FILE="$PROCEDURES_DIR/$FILENAME"
...
PROCEDURE_NAME=$(basename "$FILENAME" .sql)
