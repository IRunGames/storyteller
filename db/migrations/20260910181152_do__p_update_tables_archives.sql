...
`id_archived_by_user`, and `is_archived` columns with proper types.
...
is attached to automatically set `id_archived_by_user` to NULL when `is_archived`
...
3. For each table:
   ...
   b. Ensure the `id_archived_by_user` column exists and is of the correct type (`UUID`).
...
5. Attach a trigger to NULL the `id_archived_by_user` column automatically when
...
BEGIN
    ...
    LOOP
        ...
        IF column_type IS NULL THEN
            ...
            EXECUTE format(
                'ALTER TABLE %I ADD COLUMN id_archived_by_user UUID',
                tbl.table_name
            );
            RAISE NOTICE '"id_archived_by_user" column added to table: %', tbl.table_name;
        ELSIF UPPER(column_type) <> 'UUID' THEN
            RAISE NOTICE '"id_archived_by_user" column exists but is of type %. Updating to UUID.', column_type;
            BEGIN
                EXECUTE format(
                  'ALTER TABLE %I ALTER COLUMN id_archived_by_user TYPE uuid USING id_archived_by_user::uuid',
                  tbl.table_name);
            EXCEPTION WHEN datatype_mismatch OR cannot_coerce OR invalid_text_representation THEN
                ...
                EXECUTE format('ALTER TABLE %I DROP COLUMN id_archived_by_user',
                               tbl.table_name);
                EXECUTE format('ALTER TABLE %I ADD COLUMN id_archived_by_user uuid',
                               tbl.table_name);
            END;
            RAISE NOTICE '"id_archived_by_user" column updated in table: %', tbl.table_name;
        ELSE
            RAISE NOTICE '"id_archived_by_user" column already exists and is of the correct type in table: %', tbl.table_name;
        END IF;

        -- Ensure a single, named FK constraint to users(id_user) with
        -- ON DELETE SET NULL. id_archived_by_user is an authorship/audit
        ...
        -- names the constraint fk_<table>_id_archived_by_user, so this no longer
        -- accumulates duplicate unnamed FKs on every run.
        RAISE NOTICE 'Ensuring FK %(id_archived_by_user) -> users(id_user) ON DELETE SET NULL...', tbl.table_name;
        PERFORM _ensure_foreign_key(tbl.table_name, 'id_archived_by_user', 'users', 'id_user', 'SET NULL');
        ...
        -- Attach the trigger to nullify `id_archived_by_user`
